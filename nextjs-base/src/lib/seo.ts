...
export function buildMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'website',
      locale: alternates && alternates.length > 0 ? alternates[0].hreflang : undefined,
    },
...
export async function getPageSEO(slug: string, draft = false, locale?: string): Promise<Metadata | null> {
  try {
    ...
    const strapiBase = (process.env.NEXT_PUBLIC_STRAPI_URL ?? 'http://localhost:1337').replace(/\/$/, '')
    ...
    const alternatesArr: Hreflang[] = [
      ...
      ...localizations.map((l: Page & StrapiEntity) => ({ hreflang: l.locale || 'fr', href: `${siteBase}/${l.locale || 'fr'}/${slug}` })),
    ]
