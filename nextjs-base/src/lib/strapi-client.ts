  ...
  async findMany<T>(
    ...
    try {
      ...
      const isDraftRequest =
        options?.publicationState === 'preview' || options?.status === 'draft'
    ...
    try {
      ...
      if (options?.publicationState === 'preview') {
        try {
          ...
          const tokenLen = this.config.apiToken
            ? this.config.apiToken.length
            : 0
          console.info(
            `[diag] StrapiClient preview request — contentType=${contentType} id=${id} preview=true apiTokenPresent=${tokenPresent} tokenLen=${tokenLen} url=${url}`
          )
        } catch {}
      }
      ...
      try {
        const text = await response.text()
        console.info(
          `[diag] StrapiClient response — contentType=${contentType} id=${id} status=${response.status} contentLength=${text.length} url=${url}`
        )
        if (!response.ok) {
          try {
            const error: StrapiErrorResponse = JSON.parse(text)
