...
const TranslationBlock = ({
  ...
  const renderBlocks = (blocks: StrapiBlock[] | undefined) => {
    ...
    return blocks.map((block, index) => {
      switch (block.type) {
        case 'paragraph':
          return (
            <p key={index} className={`text-gray-700 mb-4 ${textAlignmentClasses.left}`}>
              {block.children?.map((child, childIndex) => {
                if (child.type === 'text') return <span key={childIndex}>{child.text}</span>
                return null
              })}
            </p>
          )
        case 'heading':
          ...
          return (
            <HeadingTag key={index} className={`${headingClasses[level as keyof typeof headingClasses]} ${textAlignmentClasses.left}`}>
              {block.children?.map((child, childIndex) => {
                if (child.type === 'text') return <span key={childIndex}>{child.text}</span>
                return null
              })}
              ...
              {block.children?.map((child, childIndex) => (
                <li key={childIndex} className="mb-2">
                  {Array.isArray(child.children) && child.children.map((grandChild: any, grandChildIndex: number) => {
                    if (grandChild.type === 'text') return <span key={grandChildIndex}>{grandChild.text}</span>
                    return null
                  })}
                </li>
              ))}
  ...
  const examplesToUse: ExampleItem[] = React.useMemo(() => {
    ...
    if (source || translation) return [{ source: source || [], translation: translation || [], sourceLanguage, author }]
    return []
        ...
        {examplesToUse.length > 1 && (
          <div className="mb-4 flex gap-2" role="tablist" aria-label="Examples">
            {examplesToUse.map((ex, idx) => (
              ...
              >
                {languageName(ex.sourceLanguage || sourceLanguage)}{ex.author ? ` — ${ex.author}` : ''}
              </button>
            ))}
          </div>
        )}

        <div className={`flex flex-col md:flex-row gap-6 md:divide-x md:divide-gray-200`}>
          <div className="w-full md:w-1/2 md:pr-6">
            {showLanguageLabel && (
              <div className="text-sm font-medium text-gray-600 mb-2" aria-label={`Source language: ${languageName(active.sourceLanguage || sourceLanguage)}`}>
            ...
            {(active.author || author) && <div className="text-xs text-gray-500 mt-4">{active.author || author}</div>}
          </div>

          <div className="w-full md:w-1/2 md:pl-6">
            {showLanguageLabel && (
              <div className="text-sm font-medium text-gray-600 mb-2" aria-label={`Target language: ${languageName(translationLanguage)}`}>
